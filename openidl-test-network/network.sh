...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating aais Identities"

    
    cryptogen generate --config=./organizations/cryptogen/crypto-config-aais.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating analytics Identities"

    
    cryptogen generate --config=./organizations/cryptogen/crypto-config-analytics.yaml --output="organizations"
    res=$?
    ...
    infoln "Creating carrier Identities"

    
    cryptogen generate --config=./organizations/cryptogen/crypto-config-carrier.yaml --output="organizations"
    res=$?
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/aais/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for aais and analytics"
...
# call the script to create the channel, join the peers of aais and analytics,
...
# channel profile defaults to "mychannel"
