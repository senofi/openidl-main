...
function createTargetChannelTransactions() {
    ...
    return new Map(
        targetChannelConfig.targetChannels.map(({channelName, chaincodeName}) => {
            const targetChannelTransaction = new Transaction(org, user, channelName, chaincodeName, mspId);
