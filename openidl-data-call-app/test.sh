...
# Add Cloudand and IDP credentials to local config files (this is foe execution of test cases)
cat <<< ${IDP_CONFIG} | sed -e 's/^"//' -e 's/"$//' | sed -r 's/\\/"/g' > server/config/local-cognito-config.json
