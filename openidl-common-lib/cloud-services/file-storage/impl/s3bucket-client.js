...
class S3BucketClient extends AbstractFileStorageClient {
  constructor() {
    super();
    this.bucketConfig = JSON.parse(process.env.S3_BUCKET_CONFIG) || {};
    if (isEmpty(this.bucketConfig)) {
      throw new Error(
          'Missing AWS S3 bucket configuration!');
    }
    this.bucketName = this.bucketConfig['bucketName'];
  }

  async _getClient() {
    const accessParams = await getAccessParams(this.bucketConfig);
