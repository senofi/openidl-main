...
class HashicorpVaultClient extends AbstractWalletStorageClient {
    ...
    async init() {
        const vaultSecretName = config.get("vaultConfigName");
        const secretsClient = await SecretsClientFactory.getInstance();
        const vaultConfig = await secretsClient.getSecret(vaultSecretName);
    ...
    async get(name) {
        try {
            ...
            try {
                const {data} = await nodeVault.read(`${this.vaultConfig.orgName}/data/${this.vaultConfig.vaultPath}/${name}`);
                id = data.data.id;
    ...
    async put(name, identity) {
        try {
            ...
            await nodeVault.write(`${this.vaultConfig.orgName}/data/${this.vaultConfig.vaultPath}/${name}`, {
                "data": {
                    "id": name,
                    "data": data
                }
